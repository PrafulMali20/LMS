...
const SearchPage = () => {
  ...
  const [selectedCategories, setSelectedCategories] = useState([]);  // Fixed typo here
  ...
  const handleFilterChange = (categories, price) => {
    setSelectedCategories(categories);  // Fixed typo here
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-8">
      ...
      <div className="flex flex-col md:flex-row gap-10">
        ...
        <div className="flex-1">
          {isLoading ? (
            // Dynamically show skeleton loaders based on expected course count
            Array.from({ length: data?.courses?.length || 3 }).map((_, idx) => (
